...
function toneFor(ms: number): { text: string; bar: string } {
    if (ms >= 7.5 * HOUR) return { text: 'text-success', bar: 'bg-success' };
    if (ms >= 4 * HOUR) return { text: 'text-warning', bar: 'bg-warning' };
    return { text: 'text-destructive', bar: 'bg-destructive' };
}

/**
 * Page header: today's date and a tracked-today card with avg per entry and
 * entry count — same shape as the calendar's monthly card.
 */
export function TodayHeading() {
    ...
    const dateLine = `${weekday}, ${nowDate.getDate()} ${month}`;
        ...
        <header className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex min-w-56 flex-col gap-2 rounded-2xl bg-card px-4 py-2.5 ring-1 ring-border">
                ...
                <div
                    ...
                    aria-valuenow={total}
                    aria-label="Tracked today against 8-hour target"
                >
                    <div
